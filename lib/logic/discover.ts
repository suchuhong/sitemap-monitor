...
export async function discover({ rootUrl, ownerId }: { rootUrl: string; ownerId: string }) {
  ...
  if (res.ok) {
    ...
    txt.split(/\r?\n/).forEach(l => { const m = l.match(/^\s*Sitemap:\s*(\S+)/i); if (m) smCandidates.push(m[1].trim()) })
  }
...
async function upsertSitemap(siteId: string, url: string, isIndex: boolean) {
  try {
    await db.insert(sitemaps).values({ id: randomUUID(), siteId, url, isIndex, discoveredAt: new Date() })
  } catch {}
}
