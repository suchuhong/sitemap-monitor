...
// naive auth stub
app.use("*", async (c, next) => { c.set("userId", "demo-user"); await next() })
...
app.post("/cron/scan", async (c) => {
  ...
  if (!token || token !== `Bearer ${process.env.CRON_TOKEN}`) return c.body("unauthorized", 401)
...
app.post("/sites/:id/webhooks", async (c) => {
  // Accept form or JSON
  let targetUrl = "", secret = ""
  if (c.req.header("content-type")?.includes("application/json")) {
    const b = await c.req.json() as any
    targetUrl = b.targetUrl; secret = b.secret ?? ""
  } else {
  ...
  await db.insert(webhooks).values({ id: randomUUID(), siteId: c.req.param("id"), targetUrl, secret })
  return c.json({ ok: true })
})

app.post("/sites/:id/test-webhook", async (c) => {
  const { notifyChange } = await import("@/lib/logic/notify")
  await notifyChange(c.req.param("id"), { scanId: "test", added: 1, removed: 0, updated: 0, type: "test" })
  return c.json({ ok: true })
})
...
app.get("/sites/:id/changes.csv", async (c) => {
  ...
  const rows = [["type","detail","occurredAt"]].concat(
    list.map((r:any) => [r.type, r.detail ?? "", new Date(Number(r.occurredAt||0)*1000).toISOString()])
  )
...
app.post("/sites/import", async (c) => {
  const form = await c.req.parseBody()
  let csv = ""
  if (typeof form["csv"] === "string" && form["csv"].trim()) csv = String(form["csv"]).trim()
  if ((form as any).file && typeof (form as any).file === "object" && "name" in (form as any).file) {
  ...
  for (const line of lines) {
    ...
    try { results.push(await discover({ rootUrl: line, ownerId: c.get("userId") })) } catch (e) { console.error("import fail", line, e) }
  }
...
app.get("/sites/export.csv", async (c) => {
  const { db } = await import("@/lib/db")
  const rows = await db.all(sql`SELECT id, root_url as rootUrl, robots_url as robotsUrl, created_at as createdAt FROM sites ORDER BY created_at DESC`) as any[]
  const csv = ["id,rootUrl,robotsUrl,createdAt"]
    .concat(rows.map(r => [r.id, r.rootUrl, r.robotsUrl ?? "", r.createdAt ?? ""].map(v => `"${String(v).replaceAll('"','""')}"`).join(",")))
    .join("\n")
  return c.body(csv, 200, {
    "content-type": "text/csv; charset=utf-8",
