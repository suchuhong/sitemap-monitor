...
const COLMAP: Record<string, string> = { rootUrl: "root_url", createdAt: "created_at" }

export default async function SitesPage({ searchParams }: { searchParams?: Record<string, string | string[]> }) {
  const page = getInt(searchParams, "page", 1)
  const pageSize = Math.min(getInt(searchParams, "pageSize", 10), 50)
  const sort = ["rootUrl", "createdAt"].includes(getParam(searchParams, "sort", "createdAt")) ? getParam(searchParams, "sort", "createdAt") : "createdAt"
  const dir = getParam(searchParams, "dir", "desc") === "asc" ? "asc" : "desc"

  const orderCol = COLMAP[sort]
  const offset = (page - 1) * pageSize

  const totalRow = await db.get(sql`SELECT COUNT(*) as count FROM sites`) as unknown as { count: number }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm space-x-4">
          <a href="/sites/new" className="underline">新增站点</a>
          <a href="/sites/import" className="underline">批量导入</a>
          <a href="/api/sites/export.csv" className="underline">导出 CSV</a>
        </div>
      </div>
      <SitesTableSSR data={rows as any} sort={sort} dir={dir as any} page={page} pageSize={pageSize} total={total} />
    </div>
  )
}
